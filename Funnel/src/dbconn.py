...
class DbConnection:
    """
    Class to simplify db connection management,
    ...
    def initialize(cls,
        ...
        cls._instance = DbConnection(host=host,
                                     port=port,
                                     dbname=dbname,
                                     user=user,
                                     password=password,
    ...
    def execute_commit_or_wait(self, queries: Iterable[Query]) -> Dict[Query, list[tuple]]:
        """
        ...
        while True:
            try:
                ...
                result = {}
                for q in queries:
                    cursor.execute(q.get_expression(), q.get_args())
                    if q.expect_return():
                        result[q] = cursor.fetchall()
