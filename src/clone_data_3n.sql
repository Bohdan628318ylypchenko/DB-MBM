	...
	BEGIN
		...
		LOOP
			...
			IF (zno_row.ukrtest IS NOT NULL) THEN
				...
				-- p_ti_tr
				INSERT INTO p_ti_tr
					(person_id, test_institution_id, test_result_id, year_of_attempt)
				VALUES
					(_person_id, _test_institution_id, _test_result_id, zno_row.yearofattempt);
			END IF;
...
SELECT * FROM person_location_info;
...
SELECT COUNT(*) FROM p_ti_tr;

TRUNCATE TABLE p_ti_tr CASCADE;
TRUNCATE TABLE test_result CASCADE;
TRUNCATE TABLE test_institution CASCADE;
TRUNCATE TABLE person CASCADE;
TRUNCATE TABLE educational_institution CASCADE;
TRUNCATE TABLE learning_profile CASCADE;
TRUNCATE TABLE person_location_info CASCADE;
TRUNCATE TABLE location_info CASCADE;


SELECT * FROM zno LIMIT 20;
SELECT * FROM zno WHERE chemptregname = NULL;